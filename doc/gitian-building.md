...
*Setup instructions for a Gitian build of ZOOW Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ZOOW
...
- [Building ZOOW Core](#building-zoow-core)
...
Clone the git repositories for ZOOW Core and Gitian.
...
in the ZOOW Core repository under 'Fetch and create inputs' to install sources which require
...
To build ZOOW Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ZOOW Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zoow/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/zoowpay/zoow
...
For example:
```bash
URL=https://github.com/crowning-/zoow.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit zoow=${COMMIT} --url zoow=${URL} ../zoow/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zoow=${COMMIT} --url zoow=${URL} ../zoow/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zoow=${COMMIT} --url zoow=${URL} ../zoow/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the zoow git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zoow/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/zoow
SIGPATH=/some/root/path/zoow-detached-sigs

./bin/gbuild --url zoow=${BTCPATH},signature=${SIGPATH} ../zoow/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zoow-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zoow-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zoow-osx-build.assert
```
...
[zoow/gitian.sigs](https://github.com/zoowpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
