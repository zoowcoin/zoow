...
ZOOW Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 8491 for connections and services them.
...
However, in open source software such as ZOOW Core, where everyone uses
...
pay attention to for reviewers of ZOOW Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: ZOOW Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against ZOOW Core so that they can be integrated
