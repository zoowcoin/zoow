...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in zoow.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("zoow.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* ZOOW features */
    { "zoow",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "zoow",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "zoow",               "gobject",                &gobject,                true,  {} },
    { "zoow",               "voteraw",                &voteraw,                true,  {} },
