...
const char * const BITCOIN_CONF_FILENAME = "zoow.conf";
const char * const BITCOIN_PID_FILENAME = "zoowd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "zoow" is a composite category enabling all ZOOW-related debug output
                if(ptrCategory->count(std::string("zoow"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZOOW
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZOOW
    // Mac: ~/Library/Application Support/ZOOW
    // Unix: ~/.zoow
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZOOW";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZOOW";
#else
    // Unix
    return pathRet / ".zoow";
#endif
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty zoow.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override zoow.conf
